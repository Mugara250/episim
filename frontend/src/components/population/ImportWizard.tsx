...
export function ImportWizard() {
      ...
      <div className="mt-6 max-w-xl rounded-2xl border border-border bg-surface p-6">
        ...
        {done && (
          <div className="mt-6 flex gap-3">
            <button
              type="button"
              onClick={() => dataset && navigate(`/data/population/${dataset.id}`)}
            ...
            <button
              type="button"
              onClick={() => navigate("/data/population")}
  ...
  return (
    <form onSubmit={goConfirm} className="mt-6 max-w-xl space-y-4">
      ...
      <button type="submit" className="rounded-full bg-brand px-5 py-2.5 text-sm font-semibold text-white transition hover:text-bg hover:bg-brand-light">
        Next: confirm format
      </button>
    </form>
  );
